...
def sl_err(x, y, reps=500):
    n = len(x)
    return np.std([sl(x[i], y[i]) for i in (rng.choice(n, n, replace=True) for _ in range(reps))])
...
print("=" * 84)
print(
    f'{"":8s}{"LHS (BH slope)":>18s}{"dlogf/dlogMh":>16s}{"RHS = 5/3+that":>18s}{"LHS-RHS":>12s}{"verdict":>14s}'
)
for t in ["TNG50", "EAGLE", "SIMBA"]:
    d = load(t)
    mh, bh, mg = d["DM_MASS"], d["BH_MASS"], d["GAS_MASS"]
    lo, hi = np.percentile(mh, [3, 97])
    m = (mh > lo) & (mh < hi)
    lhs = sl(mh[m], bh[m])
    elhs = sl_err(mh[m], bh[m])
    fg = mg[m] - mh[m]  # log f_gas = log Mgas - log Mhalo
    dfg = sl(mh[m], fg)
    edfg = sl_err(mh[m], fg)
    rhs = 5 / 3 + dfg
    diff = lhs - rhs
    err = np.hypot(elhs, edfg)
    v = "CONSISTENT" if abs(diff) < 3 * err else "FALSIFIED"
    print(
        f"{t:8s}{lhs:>13.3f}+-{elhs:4.3f}{dfg:>11.3f}+-{edfg:4.3f}{rhs:>18.3f}{diff:>9.3f}+-{err:4.3f}{v:>14s}"
    )
...
print(
    f'{"":8s}{"r(bh,Mh|M*)":>14s}{"r(bh,M*|Mh)":>14s}{"topology":>12s}'
