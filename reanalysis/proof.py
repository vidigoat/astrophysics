...
def load(t,lo=None,hi=None,key='STELLAR_MASS'):
    ...
    if lo is not None:
        m=(d[key]>lo)&(d[key]<hi); d={k:v[m] for k,v in d.items()}
    return d
def pc(x,y,Z):
    def r(v):
...
for t in ['TNG50','EAGLE','SIMBA']:
    ...
    for _ in range(2000):
        ...
        bb,*_=np.linalg.lstsq(X[i],y[i],rcond=None); bs.append(bb)
    bs=np.array(bs); sa,sb=bs[:,1].std(),bs[:,2].std()
...
print('#'*78)
eagle_val=pc(*[load('EAGLE',9.5,11.0)[k] for k in ['BH_MASS','STELLAR_MASS']],[load('EAGLE',9.5,11.0)['DM_MASS']])
...
for t in ['TNG50','EAGLE','SIMBA']:
    d=load(t)
    for lab,m in [('M_BH < 1e7 ',d['BH_MASS']<7.0),
                  ('1e7-1e8   ',(d['BH_MASS']>=7.0)&(d['BH_MASS']<8.0)),
                  ('M_BH > 1e8',d['BH_MASS']>=8.0)]:
        n=int(m.sum())
        if n<120: print(f'  {t:7s} {lab}  N={n:6d}   (too few)'); continue
...
for t in ['TNG50','EAGLE','SIMBA']:
    ...
    lo,hi=np.percentile(mh,[2,98]); m=(mh>lo)&(mh<hi)
    A=np.column_stack([np.ones(m.sum()),mh[m]]); b,*_=np.linalg.lstsq(A,bh[m],rcond=None)
    n=int(m.sum()); bs=[]
    for _ in range(1000):
        i=rng.choice(n,n,replace=True)
        bb,*_=np.linalg.lstsq(A[i],bh[m][i],rcond=None); bs.append(bb[1])
    print(f'  {t:7s} M_BH ~ M_halo^({b[1]:.3f} +- {np.std(bs):.3f})   over log M_halo {lo:.1f}-{hi:.1f}, N={n}')
...
for t in ['TNG50','EAGLE','SIMBA']:
    d=load(t,9.5,11.0); n=len(d['BH_MASS'])
    ...
    if abs(zh)<3 and abs(zs)>5: v='M_BH _||_ halo  GIVEN M*'
    elif abs(zs)<8 and abs(zh)>8 and abs(zh)>abs(zs): v='halo dominates'
    else: v='both non-zero'
