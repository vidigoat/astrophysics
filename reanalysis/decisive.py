...
for t,d in raw.items():
    ...
    print(f'{t}: {m.sum()} galaxies in {LO} < log M* < {HI}   '
...
PAIRS=[('BH_MASS','SSFR'),('BH_MASS','DM_MASS'),('BH_MASS','STELLAR_MASS'),
       ('BH_MASS','GAS_MASS'),('BH_MASS','VEL_DISP')]
NB=30
for pen in [3,5]:
    print('='*84)
    ...
    for t in ['TNG50','EAGLE','SIMBA','SIMBA-noisy']:
        ...
        cols=list(base.keys()); n=len(base['STELLAR_MASS']); cnt[t]={p:0 for p in PAIRS}
        for _ in range(NB):
            ...
            E=graph_at(df,pen)
            for p in PAIRS:
                if has(E,*p): cnt[t][p]+=1
    print(f'{"link to M_BH":16s}'+''.join(f'{t:>14s}' for t in ['TNG50','EAGLE','SIMBA','SIMBA-noisy']))
    for p in PAIRS:
        lbl=p[1].replace('STELLAR_MASS','M*').replace('DM_MASS','M_halo').replace('GAS_MASS','M_gas').replace('VEL_DISP','sigma')
        print(f'{lbl:16s}'+''.join(f'{100*cnt[t][p]/NB:>13.0f}%' for t in ['TNG50','EAGLE','SIMBA','SIMBA-noisy']))
    print()
