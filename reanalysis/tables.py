    ...
    for i in range(nr):
        ...
        for line in str(s.get_java()).split('\n'):
            ...
            if len(p)>=4 and p[0].rstrip('.').isdigit() and set(p[2])<=set('<->o-'):
                a,m_,b=p[1],p[2],p[3]; k=tuple(sorted([a,b])); pres[k]=pres.get(k,0)+1
...
print('='*72); print('TABLE: penalty dependence (corrected data, matched N=%d)'%N); print('='*72)
...
for pen in [3,5,8]:
    ...
    print(f'{pen:>4} {"/".join(str(len(G[t])) for t in ["TNG50","EAGLE","SIMBA"]):>20} '
          f'{len(u):>7} {len(all3):>9} {same:>9}')

print(); print('='*72); print('TABLE: within- vs cross-code agreement (corrected data)'); print('='*72)
def jac(a,b):
    sa,sb=set(a),set(b); return len(sa&sb)/max(len(sa|sb),1)
def orient(a,b):
    sh=[k for k in set(a)&set(b) if a[k] is not None and b[k] is not None]
    if not sh: return float('nan'),0
    return sum(1 for k in sh if a[k]==b[k])/len(sh), len(sh)
halves={}
for t,d in data.items():
    n=len(d['STELLAR_MASS']); p=rng.permutation(n); h=n//2
