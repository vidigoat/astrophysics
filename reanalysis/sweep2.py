...
LO,HI=9.5,11.0
V=['STELLAR_MASS','GAS_MASS','DM_MASS','BH_MASS','LOG_SIGMA',
   'STAR_METALLICITY','GAS_METALLICITY','SSFR']
NICE={'STELLAR_MASS':'M*','GAS_MASS':'Mgas','DM_MASS':'Mhalo','BH_MASS':'Mbh',
      'LOG_SIGMA':'sigma','STAR_METALLICITY':'Zstar','GAS_METALLICITY':'Zgas','SSFR':'sSFR'}
...
for a,b in itertools.combinations(V,2):
    ...
    if max(abs(v) for v in vals)<0.20: continue
    ...
    flag='SIGN FLIP (3sig)' if len(sgn)>1 else ('universal' if all(abs(v)>0.25 for v in vals) and len(set(np.sign(vals)))==1 else '')
    if flag.startswith('SIGN'): flips.append((a,b,vals,errs))
    rows.append((max(abs(v) for v in vals),a,b,vals,errs,flag))
for _,a,b,vals,errs,flag in sorted(rows,reverse=True):
    print(f'{NICE[a]+"-"+NICE[b]:18s}'+''.join(f'{v:>16.3f} +-{e:5.3f}' for v,e in zip(vals,errs))+f'  {flag}')
print()
print('='*80)
print('SIGN FLIPS surviving 3-sigma on BOTH sides:')
for a,b,vals,errs in flips:
    print(f'  {NICE[a]:7s}-{NICE[b]:7s}  ' +
