...
def sf_sample(name,fix_tng=False):
    ...
    return d
tng=pickle.load(open(P+'tng50_final.pkl','rb')); tng={k:np.asarray(v,float) for k,v in tng.items()}
# corrected TNG labels
TNG={'STELLAR_MASS':tng['GAS_MASS'],'GAS_METALLICITY':tng['GAS_METALLICITY'],'SFR':tng['SFR']}
EAG=pickle.load(open(P+'eagle_final.pkl','rb')); EAG={k:np.asarray(v,float) for k,v in EAG.items()}
SIM=pickle.load(open(P+'simba_final.pkl','rb')); SIM={k:np.asarray(v,float) for k,v in SIM.items()}
...
for t,(ms,z,ss) in S.items(): print(f'   {t:7s} N={len(ms):6d}   median logM* = {np.median(ms):.2f}')
print()
...
for lo,hi in BINS:
    ...
    for t in ['TNG50','EAGLE','SIMBA']:
        ms,z,ss=S[t]; m=(ms>lo)&(ms<hi); n=int(m.sum())
        if n<120: row+=f'{"--":>10s}{"":>7s}{n:>7d}'; continue
        v=pc(z[m],ss[m],[ms[m]])
