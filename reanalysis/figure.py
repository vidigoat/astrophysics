...
for j,t in enumerate(['TNG50','EAGLE','SIMBA']):
    ...
    m=(d['STELLAR_MASS']>LO)&(d['STELLAR_MASS']<HI); d={k:v[m] for k,v in d.items()}
    bh,ms,mh=d['BH_MASS'],d['STELLAR_MASS'],d['DM_MASS']

    # top: M_BH residual vs halo residual, both at fixed M*
    x,y=res(mh,[ms]),res(bh,[ms])
    ax=axes[0,j]; ax.scatter(x,y,s=3,alpha=.18,color=COL[t],lw=0)
    s=np.polyfit(x,y,1)[0]; r=np.corrcoef(x,y)[0,1]
    ...
    if j==0: ax.set_ylabel(r'$\Delta \log M_{\rm BH}$  (at fixed $M_\star$)')
    ax.text(.04,.93,f'slope = {s:.2f}\nr = {r:+.3f}',transform=ax.transAxes,
            va='top',fontsize=11,bbox=dict(fc='white',ec='0.7',alpha=.9))
    ax.set_xlim(np.percentile(x,.5),np.percentile(x,99.5)); ax.set_ylim(np.percentile(y,.5),np.percentile(y,99.5))

    # bottom: the mirror test - M_BH residual vs stellar residual at fixed halo
    x2,y2=res(ms,[mh]),res(bh,[mh])
    ax=axes[1,j]; ax.scatter(x2,y2,s=3,alpha=.18,color=COL[t],lw=0)
    s2=np.polyfit(x2,y2,1)[0]; r2=np.corrcoef(x2,y2)[0,1]
    ...
    if j==0: ax.set_ylabel(r'$\Delta \log M_{\rm BH}$  (at fixed $M_{\rm halo}$)')
    ax.text(.04,.93,f'slope = {s2:.2f}\nr = {r2:+.3f}',transform=ax.transAxes,
            va='top',fontsize=11,bbox=dict(fc='white',ec='0.7',alpha=.9))
    ax.set_xlim(np.percentile(x2,.5),np.percentile(x2,99.5)); ax.set_ylim(np.percentile(y2,.5),np.percentile(y2,99.5))

fig.suptitle('What does the black hole actually know about?   '
             'EAGLE: the halo.   TNG50 / SIMBA: the stars.',fontsize=14,weight='bold')
