...
print('='*80)
print(f'{"":8s}{"b":>8s}{"c":>8s}{"s1":>8s}{"s2":>8s}{"R_fork pred":>14s}{"R_chain":>10s}{"R measured":>14s}{"which":>9s}')
for t in ['TNG50','EAGLE','SIMBA']:
    d=load(t); mh,ms,bh=d['DM_MASS'],d['STELLAR_MASS'],d['BH_MASS']
    V=np.var(mh)
    b_=sl(mh,ms); c_=sl(mh,bh); s1=sd(ms,[mh]); s2=sd(bh,[mh])
    ...
    print(f'{t:8s}{b_:>8.3f}{c_:>8.3f}{s1:>8.3f}{s2:>8.3f}{R_fork:>14.3f}{1.0:>10.3f}{R_meas:>10.3f}+-{e:4.3f}{which:>9s}')
...
for t in ['TNG50','EAGLE','SIMBA']:
    ...
    a_meas=sl(ms,bh); R_meas=sd(bh,[mh])/np.sqrt(a_meas**2*sd(ms,[mh])**2+sd(bh,[ms])**2)
    n=len(mh); bs=[]
    for _ in range(400):
        i=rng.choice(n,n,replace=True); am=sl(ms[i],bh[i])
        bs.append(sd(bh[i],[mh[i]])/np.sqrt(am**2*sd(ms[i],[mh[i]])**2+sd(bh[i],[ms[i]])**2))
    e=np.std(bs)
