...
for lab,tag in [('EAGLE',None),('SIMBA',None)]:
    ...
    for lo,hi in [(10.0,10.5),(10.5,11.0),(11.0,12.0)]:
        ...
        ms=d['STELLAR_MASS'][m]; z=np.log10(d['GAS_METALLICITY'][m]); sfr=d['SFR'][m]
        sf=sfr>0
        if sf.sum()<80: print(f'   {lo}-{hi}: too few'); continue
        ...
        print(f'   {lo}-{hi}  N_SF={sf.sum():5d}   r(Z,sSFR|M*)={r_ssfr:+.3f}   r(Z,SFR|M*)={r_sfr:+.3f}   with quenched floored={r_floor:+.3f}')
