...
NRUNS=50; PEN=5; TRUNC=7; ALPHA=0.01
...
for t in ['TNG50','EAGLE','SIMBA']:
    d=load(t); df=pd.DataFrame(np.column_stack([d[c] for c in V]),columns=V)
    pres,marks={},{}
    for i in range(NRUNS):
        for a,m,b in run(df,2000+i):
            k=tuple(sorted([a,b])); pres[k]=pres.get(k,0)+1
            e=f'{a} {m} {b}' if [a,b]==list(k) else f'{b} {m[::-1].translate(str.maketrans("<>","><"))} {a}'
            marks.setdefault(k,{}); marks[k][e]=marks[k].get(e,0)+1
    rows=[]
    for k,c in sorted(pres.items(),key=lambda kv:-kv[1]):
        if c/NRUNS<0.5: continue
        best,bc=max(marks[k].items(),key=lambda kv:kv[1])
        rows.append({'edge':best,'presence':round(c/NRUNS,3),'orient_frac':round(bc/c,3)})
    out=pd.DataFrame(rows); out.to_csv(OUT+f'consensus_corrected_{t}.csv',index=False)
