...
for i,t in enumerate(['TNG50','EAGLE','SIMBA']):
    d=load(t); mh,ms,bh=d['DM_MASS'],d['STELLAR_MASS'],d['BH_MASS']
    ...
    am=sl(ms,bh); Rm=sd(bh,[mh])/np.sqrt(am**2*sd(ms,[mh])**2+sd(bh,[ms])**2)
    n=len(mh); bs=[]
    for _ in range(500):
        j=rng.choice(n,n,replace=True); a2=sl(ms[j],bh[j])
        bs.append(sd(bh[j],[mh[j]])/np.sqrt(a2**2*sd(ms[j],[mh[j]])**2+sd(bh[j],[ms[j]])**2))
    e=np.std(bs)
    ax.errorbar(i,Rm,yerr=5*e,fmt='o',ms=14,color=COL[t],capsize=6,lw=2.5,zorder=5,
                label=None)
    ax.plot(i,Rf,marker='_',ms=34,mew=3,color='0.35',zorder=4)
    ax.annotate(f'{Rm:.3f}',(i,Rm),xytext=(16,-4),textcoords='offset points',
                fontsize=11,color=COL[t],weight='bold')
    ax.annotate('fork\nprediction',(i,Rf),xytext=(-64,-16),textcoords='offset points',
...
ax.set_xticks(range(3)); ax.set_xticklabels(xs,fontsize=13,weight='bold')
for lbl,c in zip(ax.get_xticklabels(),[COL[t] for t in xs]): lbl.set_color(c)
ax.set_ylabel(r'$R=\sigma(M_{BH}|M_h)_{\rm obs}\ /\ \sigma(M_{BH}|M_h)_{\rm chain}$',fontsize=12)
ax.set_ylim(0.52,1.12); ax.set_xlim(-0.5,2.5); ax.grid(alpha=.2,axis='y')
ax.set_title('A parameter-free test of black hole growth topology\n'
