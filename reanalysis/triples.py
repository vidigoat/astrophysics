...
N={'STELLAR_MASS':'M*','DM_MASS':'Mh','BH_MASS':'Mbh','STAR_METALLICITY':'Z*',
   'GAS_METALLICITY':'Zg','SSFR':'sSFR'}
def load(t):
...
for x,y,z in itertools.permutations(V,3):
    if (x,z)!=tuple(sorted([x,z])): continue     # avoid double counting X<->Z
    vals,errs=[],[]
    for t in ['TNG50','EAGLE','SIMBA']:
        d=data[t]; n=len(d[x])
        vals.append(R(d[x],d[y],d[z]))
        errs.append(np.std([R(d[x][i],d[y][i],d[z][i]) for i in (rng.choice(n,n,replace=True) for _ in range(150))]))
...
for spread,x,y,z,vals,errs,med,dis in sorted(rows,reverse=True)[:18]:
    ...
    print(f'{N[x]+" -> "+N[y]+" -> "+N[z]:26s}'+
          ''.join(f'{v:>12.3f}+-{e:4.3f}' for v,e in zip(vals,errs))+f'  {tag}{star}')
...
for spread,x,y,z,vals,errs,med,dis in sorted(rows,reverse=True):
    if not (dis and spread>0.20): continue
    print(f'  {N[x]} -> {N[y]} -> {N[z]:6s}  ' +
          '  '.join(f'{t}:{v:.2f}{"(chain)" if m else "(bypass)"}' for t,v,m in zip(['TNG','EAG','SIM'],vals,med)))
