...
def jointfit(bh, ms, mh):
    X = np.column_stack([np.ones(len(bh)), ms, mh])
    b, *_ = np.linalg.lstsq(X, bh, rcond=None)
    return b[1], b[2]
...
print("=" * 76)
for lab, (ms, mh, bh) in [
    ("A  as submitted (both bugs)", (A_ms, A_mh, A_bh)),
    ("B  labels fixed only", (B_ms, B_mh, B_bh)),
    ("C  labels + censoring fixed", (C_ms, C_mh, C_bh)),
]:
    m = (ms > 9.5) & (ms < 11.0)
    if m.sum() < 100:
        print(f"{lab:30s}  only {int(m.sum())} galaxies in the window")
        continue
    a, b = jointfit(bh[m], ms[m], mh[m])
    r1 = pc(bh[m], mh[m], [ms[m]])
    r2 = pc(bh[m], ms[m], [mh[m]])
    print(
        f"{lab:30s} N={int(m.sum()):5d}   M_BH ~ M*^{a:+.2f} Mh^{b:+.2f}   "
...
print(
    f'{"EAGLE (never had the bug)":30s} N={int(m.sum()):5d}   M_BH ~ M*^{a:+.2f} Mh^{b:+.2f}   '
