...
def orient_vec(g1, g2):
    ...
    both = sorted(set(o1) & set(o2)); return np.array([1 if o1[k] == o2[k] else 0 for k in both])
def ci(v, B=4000):
...
def main():
    ...
    half = {}
    for c in CODES:
        o, n = std[c]; idx = np.random.RandomState(1).choice(n, 2 * NH, replace=False)
        half[(c, 1)] = consensus(frame(o, idx[:NH])); half[(c, 2)] = consensus(frame(o, idx[NH:2 * NH]))
        print(f"  {c} halves done", flush=True)
    rows = []
    for c in CODES:
        j = skel_jac(half[(c, 1)], half[(c, 2)]); v = orient_vec(half[(c, 1)], half[(c, 2)]); f, lo, hi = ci(v)
        rows.append(dict(kind="within", comp=c, skel_jac=round(j, 3), n_or=len(v), orient=round(f, 3), lo=round(lo, 3), hi=round(hi, 3)))
    for i in range(3):
        for k in range(i + 1, 3):
            a, b = CODES[i], CODES[k]
            j = skel_jac(half[(a, 1)], half[(b, 1)]); v = orient_vec(half[(a, 1)], half[(b, 1)]); f, lo, hi = ci(v)
            rows.append(dict(kind="cross", comp=f"{a}-{b}", skel_jac=round(j, 3), n_or=len(v), orient=round(f, 3), lo=round(lo, 3), hi=round(hi, 3)))
    d = pd.DataFrame(rows); d.to_csv(f"{RES}/null_matchedN.csv", index=False)
    print("\n== MATCHED N=5,000 (all within and cross) ==")
    print(d.to_string(index=False))
    wj = d[d.kind == "within"].skel_jac.mean(); cj = d[d.kind == "cross"].skel_jac.mean()
    wo = d[d.kind == "within"].orient.mean(); co = d[d.kind == "cross"].orient.mean()
