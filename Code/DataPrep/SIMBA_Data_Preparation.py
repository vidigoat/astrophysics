...
snapshot 151 = z=0) and extracts the same first-order galaxy properties used
for TNG50, plus synthetic SDSS photometry (with dust) so that r-band
luminosity and U-R colour are directly comparable to the NSA observations.

Dark-matter halo mass is taken from each galaxy's parent halo. Galaxies with no
resolved central black hole are floored (as in the TNG50 treatment).

Output: Data/simba_final.pkl  (dict of variable -> array), mirroring
tng50_final.pkl so the existing FCIT/consensus pipeline can consume it.
"""
import os
import pickle
import numpy as np
...
BH_FLOOR_LOG = -10.0   # log10(M_BH/Msun) floor for galaxies with no resolved BH
...
def main():
    ...
    with h5py.File(CAT, "r") as f:
        ...
        absmag_r = np.asarray(gd["absmag.sdss_r"])    # with dust
        absmag_u = np.asarray(gd["absmag.sdss_u"])
        sfr = np.asarray(g["sfr_100"])                # 100-Myr-averaged SFR
        central = np.asarray(g["central"]).astype(bool)
        parent = np.asarray(g["parent_halo_index"])

        # Dark-matter halo mass from the parent halo.
    ...
    # Selection: central galaxies, finite, well-resolved in stellar mass.
    ...
    print(f"SIMBA galaxies: {n} total -> {n_final} after selection (centrals, resolved)")
    for k, v in d.items():
        print(f"  {k:16s} range [{np.min(v):8.3f}, {np.max(v):8.3f}]  mean {np.mean(v):8.3f}")
