"""
Run FCIT on ALFALFA × NSA dataset with NO physical parameter cuts.
...
FITS_PATH = os.environ.get('ALFALFA_FITS_PATH', os.path.join(REPO_ROOT, 'Data', '5asfullmatch.fits'))
OUTPUT_DIR = os.path.join(REPO_ROOT, "Results")
VIZ_DIR = os.path.join(REPO_ROOT, "Plots", "CausalStructure")

print("=" * 70)
print("FCIT ANALYSIS - ALFALFA × NSA (NO CUTS)")
print("=" * 70)
...
output_file = os.path.join(OUTPUT_DIR, "fcit_nocuts_alfalfa_t14_p50.txt")
with open(output_file, 'w') as f:
    f.write("FCIT Causal Graph - ALFALFA × NSA (NO CUTS)\n")
    f.write("=" * 70 + "\n")
    f.write(f"Dataset: ALFALFA x NSA (no physical parameter cuts)\n")
    f.write(f"Galaxies: {len(df):,}\n\n")
    f.write("Parameters:\n")
    f.write("  Truncation limit:  14\n")
    f.write("  Penalty discount:  50\n")
    f.write("  p-value threshold: 0.01\n\n")
    f.write("Properties:\n")
...
base_name = "fcit_nocuts_alfalfa_t14_p50"
