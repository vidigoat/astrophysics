"""
Hyperparameter sensitivity analysis for TNG50 dataset.
...
# Optimal: t=7, p=15.  Wide sweep matching Desmond & Ramsey (2025) Fig. 3 range.
TRUNCATION_LIMITS = [7]
PENALTY_DISCOUNTS = [5, 8, 12, 15, 20, 25, 30, 40, 50, 75, 100, 150, 200]
ALPHAS = [0.01]
...
# TNG50 properties (14 properties matching real dataset)
PROPERTY_NAMES = [
    "DM_MASS",
    "STELLAR_MASS",
    "GAS_MASS",
...
def generate_mlp(n_inputs, n_outputs, n_hidden_layers=4, n_neurons=50, seed=None):
    """Generate random MLP with He initialization."""
...
def generate_random_dag(n_nodes, min_edges, max_edges, seed=None):
    """Generate random DAG with specified edge count."""
    rng = np.random.RandomState(seed)
    n_edges = rng.randint(min_edges, max_edges + 1)
    G = nx.DiGraph()
    G.add_nodes_from(range(n_nodes))

    valid_edges = [(i, j) for i in range(n_nodes) for j in range(i + 1, n_nodes)]
...
def generate_mock_data(dag, n_samples, property_names, seed=None):
    """Generate mock data from DAG using MLP-based causal relationships."""
    rng = np.random.RandomState(seed)
    ...
    for node in topological_order:
        ...
        if len(parents) == 0:
            ...
            mlps[node] = generate_mlp(len(parents), 1, N_HIDDEN_LAYERS, N_NEURONS_PER_LAYER, mlp_seed)
    ...
    for node in topological_order:
        ...
        if len(parents) == 0:
            ...
            noise = beta.ppf(uniform, 2, 5)
            data[:, node] = (noise - 0.5) * 10
        else:
            parent_values = data[:, parents]
            mlp_output = forward_pass(parent_values, mlps[node]).flatten()
            if np.std(mlp_output) > 0:
                mlp_output = mlp_output / np.std(mlp_output) * np.std(parent_values[:, 0])
            uniform = rng.random(size=n_samples)
            noise = beta.ppf(uniform, 2, 5)
            noise_scaled = (noise - 0.5) * 0.5
...
def main():
    mock_datasets_path = os.path.join(MOCK_DATA_DIR, "mock_datasets.pkl")
    ...
    print(f"\n{'='*70}")
    print(f"Starting hyperparameter sensitivity analysis")
                ...
                for mock_data in mock_datasets[:N_MOCK_DATASETS]:
                    ...
                    try:
                        ...
                        search.use_basis_function_bic(
                            truncation_limit=trunc_limit, penalty_discount=penalty_discount
                        )
                        search.run_fcit()
                        ...
                        if dataset_count % 10 == 0 or dataset_count == N_MOCK_DATASETS:
                            print(
                                f"  Completed {dataset_count}/{N_MOCK_DATASETS} datasets (Run {run_num}/{total_runs}, Success: {successful_runs}, Failed: {failed_runs})"
                            )
                        ...
                        if "JVMNotRunning" in error_msg or "JVM" in error_msg:
                            ...
                            if dataset_count <= 3 or dataset_count % 10 == 0:
                                print(
                                    f"  Error at dataset {dataset_count}: {type(e).__name__}: {error_msg[:100]}"
                                )
                        continue

                print(
                    f"Finished: t={trunc_limit}, p={penalty_discount}, α={alpha} ({dataset_count} datasets)"
                )
        ...
        for complexity_cat in results_df["complexity_category"].unique():
            for trunc_limit in TRUNCATION_LIMITS:
                for penalty_discount in PENALTY_DISCOUNTS:
                    for alpha in ALPHAS:
                        mask = (
                            (results_df["size_category"] == size_cat)
    ...
    results_df.to_csv(os.path.join(RESULTS_DIR, "mock_data_hyperparameter_sensitivity.csv"), index=False)
    summary_df.to_csv(os.path.join(RESULTS_DIR, "mock_data_hyperparameter_summary.csv"), index=False)
    if len(stratified_df) > 0:
        stratified_df.to_csv(os.path.join(RESULTS_DIR, "mock_data_stratified_analysis.csv"), index=False)
    ...
    print(
        f"Best hyperparameters: t={best['truncation_limit']}, p={best['penalty_discount']}, "
