...
def main():
    ...
    within_j = {c: [] for c in CODES}
    within_o = {c: [] for c in CODES}
    cross_j, cross_o = [], []
    for b in range(B):
        halves = {}
        for c in CODES:
            o, n = std[c]
            idx = np.random.RandomState(1000 + b).choice(n, 2 * NH, replace=False)
            halves[(c, 1)] = consensus(frame(o, idx[:NH]))
            halves[(c, 2)] = consensus(frame(o, idx[NH : 2 * NH]))
        ...
        if (b + 1) % 10 == 0:
            print(f"  {b+1}/{B} partitions", flush=True)

    w_j = np.concatenate([within_j[c] for c in CODES])
    c_j = np.array(cross_j)
    ...
    rows = []
    for c in CODES:
        m, lo, hi = med_ci(within_j[c])
        rows.append(dict(quant="within_skel", grp=c, median=round(m, 3), lo=round(lo, 3), hi=round(hi, 3)))
    m, lo, hi = med_ci(c_j)
    rows.append(dict(quant="cross_skel", grp="all", median=round(m, 3), lo=round(lo, 3), hi=round(hi, 3)))
    m, lo, hi = med_ci(w_o)
    rows.append(dict(quant="within_orient", grp="all", median=round(m, 3), lo=round(lo, 3), hi=round(hi, 3)))
    m, lo, hi = med_ci(c_o)
    rows.append(dict(quant="cross_orient", grp="all", median=round(m, 3), lo=round(lo, 3), hi=round(hi, 3)))
    ...
    print(
        f"  SKELETON within (pooled): median {np.median(w_j):.2f}  95% [{np.percentile(w_j,2.5):.2f}-{np.percentile(w_j,97.5):.2f}]"
    )
    print(
        f"  SKELETON cross  (pooled): median {np.median(c_j):.2f}  95% [{np.percentile(c_j,2.5):.2f}-{np.percentile(c_j,97.5):.2f}]"
    )
    print(f"  permutation test within>cross skeleton: dObs={obs:.3f}  p={pval:.4f}")
    print(
        f"  ORIENTATION within: median {np.median(w_o):.2f}  95% [{np.percentile(w_o,2.5):.2f}-{np.percentile(w_o,97.5):.2f}]"
    )
    print(
        f"  ORIENTATION cross : median {np.median(c_o):.2f}  95% [{np.percentile(c_o,2.5):.2f}-{np.percentile(c_o,97.5):.2f}]  (chance 0.50)"
    )
    print("=" * 64)
    print("DONE")
