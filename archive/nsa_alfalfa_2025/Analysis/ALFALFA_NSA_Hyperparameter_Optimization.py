    ...
    for node in topological_order:
        ...
        if len(parents) == 0:
            ...
            if np.std(mlp_output) > 0:
                mlp_output = (
                    mlp_output / np.std(mlp_output) * np.std(parent_values[:, 0])
                    if len(parents) > 0
                    else mlp_output
                )
...
def main():
    ...
    results_df.to_csv(
        os.path.join(RESULTS_DIR, "alfalfa_nsa_mock_data_hyperparameter_sensitivity.csv"), index=False
    )
    summary_df.to_csv(
        os.path.join(RESULTS_DIR, "alfalfa_nsa_mock_data_hyperparameter_summary.csv"), index=False
    )
    if len(stratified_df) > 0:
        stratified_df.to_csv(
            os.path.join(RESULTS_DIR, "alfalfa_nsa_mock_data_stratified_analysis.csv"), index=False
        )
