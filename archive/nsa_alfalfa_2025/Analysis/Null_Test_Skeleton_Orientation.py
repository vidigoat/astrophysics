...
def load(code):
    d = pickle.load(open(f"{DATA}/{code.lower()}_final.pkl", "rb"))
    o = {}
...
def main():
    ...
    for c in CODES:
        o, n = std[c]
        idx = np.random.RandomState(1).choice(n, 10000, replace=False)
        g5[(c, 1)] = consensus(frame(o, idx[:5000]))
        g5[(c, 2)] = consensus(frame(o, idx[5000:10000]))
    print("\n== WITHIN-CODE, disjoint 5k, ORIENTATION agreement (oriented-in-both) ==")
    for c in CODES:
        v = orient_vec(g5[(c, 1)], g5[(c, 2)])
        f, lo, hi = ci(v)
        j = skel_jac(g5[(c, 1)], g5[(c, 2)])
        rows.append(
            dict(
                kind="within5k",
                comp=c,
    ...
    o, n = std["SIMBA"]
    idx = np.random.RandomState(2).choice(n, 20000, replace=False)
    gs1 = consensus(frame(o, idx[:10000]))
    gs2 = consensus(frame(o, idx[10000:20000]))
    v = orient_vec(gs1, gs2)
    f, lo, hi = ci(v)
    j = skel_jac(gs1, gs2)
    rows.append(
        dict(
    ...
    g10 = {
        c: consensus(frame(std[c][0], np.random.RandomState(42).choice(std[c][1], 10000, replace=False)))
        for c in CODES
    }
    print("\n== CROSS-CODE at 10k, ORIENTATION agreement (oriented-in-both) ==")
    for i in range(3):
        for j2 in range(i + 1, 3):
            a, b = CODES[i], CODES[j2]
            v = orient_vec(g10[a], g10[b])
            f, lo, hi = ci(v)
            jc = skel_jac(g10[a], g10[b])
            rows.append(
                dict(
                    kind="cross10k",
                    comp=f"{a}-{b}",
            ...
            print(
                f"  {a}-{b}: orient {int(v.sum())}/{len(v)}={f:.2f} [{lo:.2f}-{hi:.2f}]  skelJaccard={jc:.2f}"
            )
