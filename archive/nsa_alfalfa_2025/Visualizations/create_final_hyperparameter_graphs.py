...
def make_figure(csv_name, dataset_label, opt_t, opt_p, out_name):
    ...
    for col, c in [("precision", C_PREC), ("recall", C_REC), ("f1", C_F1)]:
        ax.fill_between(x, p16[col].values, p84[col].values, color=c, alpha=ALPHA_BAND, linewidth=0)
    ...
    ax.tick_params(
        axis="both", which="major", direction="in", length=4, width=0.8, labelsize=10, top=True, right=True
    )
    ax.tick_params(axis="both", which="minor", direction="in", length=2.5, width=0.6, top=True, right=True)
...
def main():
    make_figure(
        "mock_data_hyperparameter_sensitivity.csv",
        "TNG50",
        opt_t=7,
        opt_p=15,
    ...
    make_figure(
        "alfalfa_nsa_mock_data_hyperparameter_sensitivity.csv",
        "ALFALFA×NSA",
        opt_t=7,
        opt_p=35,
