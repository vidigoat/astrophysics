...
for t, fn, cl_n, cl_pct, cl_seed in [
    ("TNG50", "tng50_final", 10992, 66.0, 3740),
    ...
    n = len(d["BH_MASS"])
    fl = int(np.sum(d["BH_MASS"] <= -9.99))
    chk(f"{t} N", cl_n, n, 0)
    chk(f"{t} floor %", cl_pct, 100 * fl / n, 0.1)
    chk(f"{t} seeded", cl_seed, n - fl, 0)
...
for t, ca, cb in [("TNG50", 0.97, -0.04), ("EAGLE", 0.22, 0.98), ("SIMBA", 1.12, 0.16)]:
    d = load(t)
    X = np.column_stack([np.ones(len(d["BH_MASS"])), d["STELLAR_MASS"], d["DM_MASS"]])
    b, *_ = np.linalg.lstsq(X, d["BH_MASS"], rcond=None)
    chk(f"{t} a (stellar)", ca, b[1], 0.015)
    chk(f"{t} b (halo)", cb, b[2], 0.015)

print("\n--- SCREENING TABLE ---")
for t, vals in [
    ("TNG50", (0.876, 0.799, 0.599, -0.063)),
    ("EAGLE", (0.863, 0.182, 0.916, 0.624)),
    ("SIMBA", (0.677, 0.388, 0.604, 0.058)),
]:
    d = load(t)
    bh, ms, mh = d["BH_MASS"], d["STELLAR_MASS"], d["DM_MASS"]
    for lab, cl, act in [
        ("r(bh,M*)", vals[0], pc(bh, ms, [])),
        ("r(bh,M*|Mh)", vals[1], pc(bh, ms, [mh])),
        ("r(bh,Mh)", vals[2], pc(bh, mh, [])),
        ("r(bh,Mh|M*)", vals[3], pc(bh, mh, [ms])),
    ]:
...
for t, c1, c2 in [("TNG50", 0.0, 42.1), ("EAGLE", 18.6, 1.9), ("SIMBA", 0.1, 10.2)]:
    ...
    c = sd(bh, [d["DM_MASS"]])
    e = sd(bh, [d["DM_MASS"], d["STELLAR_MASS"], d["LOG_SIGMA"]])
...
for t, cl in [("TNG50", 0.976), ("EAGLE", -0.130), ("SIMBA", 0.057)]:
    d = load(t)
    rest = [d[k] for k in V if k not in ("DM_MASS", "HALFMASS_RAD")]
...
for t, cl in [("TNG50", -0.37), ("EAGLE", 0.08), ("SIMBA", -0.47)]:
    ...
    chk(
        f"{t} r(bh,Mgas|rest)",
        cl,
        pc(d["BH_MASS"][m], d["GAS_MASS"][m], [d["STELLAR_MASS"][m], d["DM_MASS"][m], d["LOG_SIGMA"][m]]),
        0.02,
    )
