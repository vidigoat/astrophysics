...
LAB = {
    "STELLAR_MASS": r"$\log M_\star$",
    "DM_MASS": r"$\log M_{\rm h}$",
    "GAS_MASS": r"$\log M_{\rm gas}$",
    "BH_MASS": r"$\log M_{\rm BH}$",
...
n = len(X["STELLAR_MASS"])
print("N =", n)
for k in V:
    print("  %-18s median %7.2f" % (k, np.median(X[k])))
...
for i in range(m):
    for j in range(m):
        ...
        if j > i:
            ...
            continue
        if i == j:
            a.hist(X[V[i]], bins=40, color="#B8E0B8", edgecolor="#1B3D1B", lw=0.6)
            a.set_yticks([])
        else:
            ...
            try:
                ...
                if len(lv) > 1:
                    a.contourf(XX, YY, Z, levels=lv + [Z.max()], colors=C[-len(lv) :])
            except Exception:
                a.plot(x, y, ".", ms=1, color="#4A9D4E", alpha=0.3)
        a.set_xlim(*LIM[V[j]])
        if i != j:
            a.set_ylim(*LIM[V[i]])
        if i == m - 1:
            a.set_xlabel(LAB[V[j]], fontsize=11)
        else:
            a.set_xticklabels([])
        if j == 0 and i > 0:
            a.set_ylabel(LAB[V[i]], fontsize=11)
        elif j > 0:
            a.set_yticklabels([])
