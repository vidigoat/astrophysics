...
def graph(d, idx, pen):
    ...
    for v in [x for x in V if x != "DM_MASS"]:
        s.add_to_tier(1, v)
...
for pen in [3, 5, 8, 12]:
    ...
    inv = [k for k in allthree if len({G[t][k] for t in G}) == 1 and any(c in G["TNG50"][k] for c in "<>")]
    print(
        "%3d  %-14s %6d %14d %10d"
        % (
            pen,
            "/".join(str(len(G[t])) for t in ["TNG50", "EAGLE", "SIMBA"]),
            len(union),
            len(allthree),
            len(inv),
        )
