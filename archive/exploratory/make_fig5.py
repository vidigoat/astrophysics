...
for t in ["TNG50", "EAGLE", "SIMBA"]:
    ...
    vals[t] = pcorr(
        d["BH_MASS"][m], d["GAS_MASS"][m], [d["STELLAR_MASS"][m], d["DM_MASS"][m], d["LOG_SIGMA"][m]]
    )
ax.bar(list(vals), [vals[t] for t in vals], color=[COL[t] for t in vals], width=0.6)
ax.axhline(0, color="k", lw=0.9)
for i, t in enumerate(vals):
    ax.text(i, vals[t] + (0.02 if vals[t] > 0 else -0.045), f"{vals[t]:+.2f}", ha="center", fontsize=11)
...
ax.set_ylim(min(vals.values()) - 0.15, max(vals.values()) + 0.15)
...
for t in ["TNG50", "EAGLE", "SIMBA"]:
    ...
    for lo, hi in edges:
        m = (
            (d["STELLAR_MASS"] > lo)
            & (d["STELLAR_MASS"] < hi)
            & np.isfinite(d["GAS_METALLICITY"])
            & np.isfinite(d["SSFR"])
        )
        if m.sum() < 40:
            continue
        xs.append((lo + hi) / 2)
        ys.append(pcorr(d["GAS_METALLICITY"][m], d["SSFR"][m], [d["STELLAR_MASS"][m]]))
...
print("saved")
print("left panel:", {k: round(v, 3) for k, v in vals.items()})
