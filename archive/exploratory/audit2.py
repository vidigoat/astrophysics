...
for t, cR, cF in [("TNG50", 1.021, None), ("EAGLE", 0.648, 0.614), ("SIMBA", 0.980, None)]:
    ...
    a = sl(ms, bh)
    R = sd(bh, [mh]) / np.sqrt(a * a * sd(ms, [mh]) ** 2 + sd(bh, [ms]) ** 2)
...
for lo, hi, cl in [(10.5, 11.0, 0.23), (11.0, 11.5, 0.73), (11.5, 12.0, 0.84), (12.0, 12.5, 1.21)]:
    ...
    chk(
        f"exponent {lo}-{hi}",
        cl,
        sl(res(d["DM_MASS"][m], [d["STELLAR_MASS"][m]]), res(d["BH_MASS"][m], [d["STELLAR_MASS"][m]])),
        0.02,
    )
...
def prep(fn):
    ...
    return (
        d["STELLAR_MASS"][ok],
        np.log10(d["GAS_METALLICITY"][ok]),
        np.log10(d["SFR"][ok]) - d["STELLAR_MASS"][ok],
    )
...
bh, ms, mh = d["BH_MASS"], d["STELLAR_MASS"], d["DM_MASS"]
e = load("EAGLE")
target = pc(e["BH_MASS"], e["STELLAR_MASS"], [e["DM_MASS"]])
