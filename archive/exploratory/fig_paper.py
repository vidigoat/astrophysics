...
POS = {
    ...
    "GAS_MASS": (-1.5, 0.95),
    "SSFR": (1.5, 0.95),
    "STAR_METALLICITY": (-0.85, -1.5),
    "GAS_METALLICITY": (0.85, -1.5),
}
...
for ax, t in zip(axes, ["TNG50", "EAGLE", "SIMBA"]):
    ...
    nx.draw_networkx_nodes(
        G, POS, ax=ax, node_size=1500, node_color="white", edgecolors=COL[t], linewidths=2.2
    )
...
for a in axes:
    a.text(
        0,
        -2.02,
        "gold: BH$-$halo    black: BH$-$galaxy ($M_\\star$ or $\\sigma$)",
        ha="center",
        fontsize=9.5,
        color="0.35",
    )
fig.suptitle(
    "Consensus causal graphs after correction. In TNG50 and SIMBA the black hole attaches to the galaxy;\n"
    "in EAGLE it attaches to the halo instead.",
    fontsize=13.5,
    weight="bold",
)
fig.tight_layout(rect=[0, 0, 1, 0.92])
fig.savefig(R + "fig1_pags.png", dpi=165)
print("fig1 done")
...
for t in ["TNG50", "EAGLE", "SIMBA"]:
    d = load(t)
    m = (d["STELLAR_MASS"] > 9.5) & (d["STELLAR_MASS"] < 11.0) & (d["DM_MASS"] < 11.5)
    f = lambda i: pc(
        d["BH_MASS"][m][i],
        d["GAS_MASS"][m][i],
...
def prep(d):
    ...
    ok = (s > 0) & (z > 0)
    return ms[ok], np.log10(z[ok]), np.log10(s[ok]) - ms[ok]
...
a2.axvspan(10.5, 12, color="goldenrod", alpha=0.13)
...
a2.grid(alpha=0.2)
fig.tight_layout()
fig.savefig(R + "fig5_feedback_fmr.png", dpi=165)
print("fig5 done")
