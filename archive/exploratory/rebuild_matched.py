...
for e in union:
    ...
    if len(inn) == 3:
        ...
        if len(dirs) == 1 and None not in dirs:
            ...
            maj += 0
            conf += 0
            invlist.append(e) if False else None
        else:
            conf += 1
            conflist.append(e)
    elif len(inn) == 2:
...
print("\n  invariant edges:")
[print("    ", a, "->", b) for (a, b) in invlist]
print("  conflicting edges:")
[print("    ", a, "-", b) for (a, b) in conflist]
...
for _ in range(NPART):
    ...
    for t in data:
        wi.append(jac(halves[t][0], halves[t][1]))
    for a, b in itertools.combinations(data, 2):
        cr.append(jac(halves[a][0], halves[b][0]))
print(
    f"  within-code Jaccard median {np.median(wi):.2f}  (16-84%: {np.percentile(wi,16):.2f}-{np.percentile(wi,84):.2f})"
)
print(
    f"  cross-code  Jaccard median {np.median(cr):.2f}  (16-84%: {np.percentile(cr,16):.2f}-{np.percentile(cr,84):.2f})"
)
