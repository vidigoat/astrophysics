...
wi = {t: [] for t in data}
cr = {p: [] for p in itertools.combinations(data, 2)}
wo = {t: [] for t in data}
co = {p: [] for p in itertools.combinations(data, 2)}
...
for t in data:
    print("%-22s %8.2f %8s" % ("Within " + t, med(wi[t]), ("%.2f" % med(wo[t])) if wo[t] else "--"))
for a, b in cr:
    print(
        "%-22s %8.2f %8s"
        % (f"{a} vs {b}", med(cr[(a, b)]), ("%.2f" % med(co[(a, b)])) if co[(a, b)] else "--")
    )
allw = [v for t in wi for v in wi[t]]
allc = [v for p in cr for v in cr[p]]
print(
    f"\nwithin-code Jaccard median {np.median(allw):.2f} "
    f"(16-84%: {np.percentile(allw,16):.2f}-{np.percentile(allw,84):.2f})"
)
print(
    f"cross-code  Jaccard median {np.median(allc):.2f} "
