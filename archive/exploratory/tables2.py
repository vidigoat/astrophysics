...
for t in data:
    ...
    allw += list(v)
    print(
        f'{"Within "+t:<24}{np.median(v):>16.2f}{f"{np.percentile(v,16):.2f}-{np.percentile(v,84):.2f}":>18}'
    )
allc = []
for (a, b), v in cr.items():
    ...
    allc += list(v)
    print(
        f'{a+" vs "+b:<24}{np.median(v):>16.2f}{f"{np.percentile(v,16):.2f}-{np.percentile(v,84):.2f}":>18}'
    )
print()
print(
    f"  OVERALL within-code median = {np.median(allw):.2f}  ({np.percentile(allw,16):.2f}-{np.percentile(allw,84):.2f})"
)
print(
    f"  OVERALL cross-code  median = {np.median(allc):.2f}  ({np.percentile(allc,16):.2f}-{np.percentile(allc,84):.2f})"
)
