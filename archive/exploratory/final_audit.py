...
print("=" * 62)
print("1. JOINT REGRESSION  log M_BH = a logM* + b logM_h + c")
print("=" * 62)
for t, d in D.items():
    m = W(d)
    X = np.column_stack([np.ones(m.sum()), d["STELLAR_MASS"][m], d["DM_MASS"][m]])
    y = d["BH_MASS"][m]
...
print("=" * 62)
print("2. SCREENING  r(M_BH,M_h) raw -> | M*   and mirror")
print("=" * 62)
for t, d in D.items():
    m = W(d)
...
print("=" * 62)
print("3. VARIANCE RATIO R (chain=1)")
print("=" * 62)
for t, d in D.items():
    m = W(d)
    ...
    rhs = a**2 * np.var(ms - np.polyval(np.polyfit(mh, ms, 1), mh)) + np.var(
        bh - np.polyval(np.polyfit(ms, bh, 1), ms)
    )
    Rs = []
    for _ in range(600):
        i = rng.integers(0, m.sum(), m.sum())
        ...
        r = a2**2 * np.var(ms[i] - np.polyval(np.polyfit(mh[i], ms[i], 1), mh[i])) + np.var(
            bh[i] - np.polyval(np.polyfit(ms[i], bh[i], 1), ms[i])
        )
        Rs.append(l / r)
    print(f"  {t:6s} R = {lhs/rhs:.3f} +/- {np.std(Rs):.3f}")

print("=" * 62)
print("4. SCATTER REDUCTION  (does adding M_h help, given M* and sigma?)")
print("=" * 62)
for t, d in D.items():
    m = W(d)
    y = d["BH_MASS"][m]
    ...
    gal = sd([d["STELLAR_MASS"][m], d["LOG_SIGMA"][m]])
    both = sd([d["STELLAR_MASS"][m], d["LOG_SIGMA"][m], d["DM_MASS"][m]])
    halo = sd([d["DM_MASS"][m]])
    print(
        f"  {t:6s} halo adds {100*(1-both/gal):5.1f}%   galaxy adds once halo known {100*(1-both/halo):5.1f}%"
    )

print("=" * 62)
print("5. FEEDBACK  r(M_BH,M_gas|M*,M_h,sigma), M_h<11.5")
print("=" * 62)
for t, d in D.items():
    m = (d["DM_MASS"] < 11.5) & np.isfinite(d["BH_MASS"]) & np.isfinite(d["GAS_MASS"])
    print(
        f"  {t:6s} {pcorr(d['BH_MASS'][m],d['GAS_MASS'][m],[d['STELLAR_MASS'][m],d['DM_MASS'][m],d['LOG_SIGMA'][m]]):+.3f}  (N={m.sum()})"
    )

print("=" * 62)
print("6. CENSORING REVERSAL (TNG50, r(M_BH,M_h|M*))")
del D
print("=" * 62)
for tag, lab in [("clean", "seeded & star-forming"), ("full", "all subhaloes incl. floored")]:
    d = dict(pickle.load(open(f"data/TNG50_{tag}.pkl", "rb")))
    g = {k: np.asarray(v, float) for k, v in d.items()}
