...
for t in COL:
    ...
    for lo, hi in bins:
        ...
        bs = [
            pcorr(d["BH_MASS"][m][i], d["DM_MASS"][m][i], [d["STELLAR_MASS"][m][i]])
    ...
    for lo, hi in bins:
        ...
        bs = [
            pcorr(d["BH_MASS"][m][i], d["STELLAR_MASS"][m][i], [d["DM_MASS"][m][i]])
            for i in (rng.integers(0, m.sum(), m.sum()) for _ in range(120))
        ]
        xs2.append((lo + hi) / 2)
        ys2.append(v)
        es2.append(np.std(bs))
    axes[1].errorbar(xs2, ys2, yerr=es2, fmt="o-", color=COL[t], label=t, lw=2, ms=6, capsize=3)
for ax, ttl, yl in [
    (axes[0], r"Halo at fixed galaxy", r"$r(M_{\rm BH},M_{\rm h}\,|\,M_\star)$"),
    (axes[1], r"Galaxy at fixed halo", r"$r(M_{\rm BH},M_\star\,|\,M_{\rm h})$"),
]:
    ax.axhline(0, color="k", lw=0.9)
    ax.axvspan(10.5, 11.5, color="0.90", zorder=0)
...
plt.tight_layout()
fig.savefig("../paper/Plots/mass_trend.png", dpi=300, bbox_inches="tight", facecolor="white")
