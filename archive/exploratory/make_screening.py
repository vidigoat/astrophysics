...
def resid(y, Z):
    X = np.column_stack([np.ones(len(y))] + list(Z))
...
for j, t in enumerate(CODES):
    d = load(t)
    m = (d["STELLAR_MASS"] > 9.5) & (d["STELLAR_MASS"] < 11.0) & np.isfinite(d["BH_MASS"])
    ms, mh, bh = d["STELLAR_MASS"][m], d["DM_MASS"][m], d["BH_MASS"][m]
    for i, (cond, xlab, ylab, title) in enumerate(
        [
            (
                [ms],
                r"$\Delta\log M_{\rm h}$ at fixed $M_\star$",
                r"$\Delta\log M_{\rm BH}$ at fixed $M_\star$",
                "halo at fixed galaxy",
            ),
            (
                [mh],
                r"$\Delta\log M_\star$ at fixed $M_{\rm h}$",
                r"$\Delta\log M_{\rm BH}$ at fixed $M_{\rm h}$",
                "galaxy at fixed halo",
            ),
        ]
    ):
        ax = axes[i, j]
        x = resid(mh if i == 0 else ms, cond)
        y = resid(bh, cond)
        ax.plot(x, y, ".", ms=1.6, alpha=0.28, color="0.35")
        ...
        ax.text(0.04, 0.93, f"slope {b:+.2f}\n$r$ = {r:+.3f}", transform=ax.transAxes, fontsize=10, va="top")
        ax.set_xlabel(xlab, fontsize=10)
        ax.set_ylabel(ylab if j == 0 else "", fontsize=10)
...
plt.tight_layout()
fig.savefig("../paper/Plots/bh_screening.png", dpi=300, bbox_inches="tight", facecolor="white")
